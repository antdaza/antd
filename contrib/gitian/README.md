...
*Setup instructions for a Gitian build of Antd using a VM or physical system.*

Gitian is the deterministic build process that is used to build the Antd CLI
...
```bash
cp antd/contrib/gitian/gitian-build.py .
```

Setup the required environment, you only need to do this once:

```
./gitian-build.py --setup antd-user x.x.x
```

Where `antd-user` is your Github name and `x.x.x` is the version tag you want to build.
...
```
git clone git@github.com:antdaza/gitian.sigs.git
git remote add antd-user git@github.com:antd-user/gitian.sigs.git
```
...
To build the most recent tag:
 ...
 `./gitian-build.py --detach-sign --no-commit -b antd-user 2.0.0`
...
```
export NAME=antd-user
export VERSION=2.0.0
gpg --output $VERSION-linux/$NAME/antd-linux-$VERSION-build.assert.sig --detach-sign $VERSION-linux/$NAME/antd-linux-$VERSION-build.assert
gpg --output $VERSION-osx-unsigned/$NAME/antd-osx-$VERSION-build.assert.sig --detach-sign $VERSION-osx-unsigned/$NAME/antd-osx-$VERSION-build.assert
gpg --output $VERSION-win-unsigned/$NAME/antd-win-$VERSION-build.assert.sig --detach-sign $VERSION-win-unsigned/$NAME/antd-win-$VERSION-build.assert
```

Make a pull request (both the `.assert` and `.assert.sig` files) to the
[antdaza/gitian.sigs](https://github.com/antdaza/gitian.sigs/) repository:
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/antd-linux-*-build.assert
    gpg --detach-sign ${VERSION}-win-unsigned/${SIGNER}/antd-win-*-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/antd-osx-*-build.assert
```
