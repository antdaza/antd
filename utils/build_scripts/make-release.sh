...
curl --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/api/v4/projects/7515512/jobs" |
  ...
  do
    ...
    curl -L --header 'PRIVATE-TOKEN: '"$TOKEN" "https://gitlab.com/antdproject/antd/-/jobs/$id/artifacts/download" -o artifacts-$build.zip
    unzip artifacts-$build.zip
    mv build/release/bin antd-$build-x64-$version
    zip -r antd-$build-x64-$version.zip antd-$build-x64-$version
  done

echo '#### sha256sum'
sha256sum antd-*-x64-$version.zip
