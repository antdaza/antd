...
#ifndef ANTD_INTEGRATION_TEST_HOOKS_H
#define ANTD_INTEGRATION_TEST_HOOKS_H
...
#endif // ANTD_INTEGRATION_TEST_HOOKS_H

//
// CPP Implementation
//
#ifdef ANTD_INTEGRATION_TEST_HOOKS_IMPLEMENTATION
#include <string.h>
#include <assert.h>
#include <chrono>
#include <thread>
...
, "antd-default-integration-test-mem-name"
, false
};
...
void antd::use_standard_cout()   { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_std_cout); }
void antd::use_redirected_cout() { if (!global_std_cout) { global_std_cout = std::cout.rdbuf(); } std::cout.rdbuf(global_redirected_cout.rdbuf()); }

void antd::init_integration_test_context(const std::string &base_name)
{
  ...
  while (global_stdin_shared_mem->Open() != 0)
  {
    ...
    {
      once_only = false;
      printf("Antd Integration Test: Shared memory %s has not been created yet, blocking ...\n", global_stdin_shared_mem->Path().c_str());
    }
  }
  ...
  if (!global_stdin_semaphore_handle)  fprintf(stderr, "Antd Integration Test: Failed to initialise global_stdin_semaphore_handle\n");
  if (!global_stdout_semaphore_handle) fprintf(stderr, "Antd Integration Test: Failed to initialise global_stdout_semaphore_handle\n");
  if (!global_stdout_ready_semaphore) fprintf(stderr, "Antd Integration Test: Failed to initialise global_stdout_ready_semaphore_handle\n");
  if (!global_stdin_ready_semaphore) fprintf(stderr, "Antd Integration Test: Failed to initialise global_stdin_ready_semaphore_handle\n");

  printf("Antd Integration Test: Hooks initialised into shared memory, %s, %s, %s, %s, %s, %s\n",
      stdin_name.c_str(),
      stdout_name.c_str(),
...
void antd::deinit_integration_test_context()
{
...
std::vector<std::string> antd::separate_stdin_to_space_delim_args(antd::fixed_buffer const *cmd)
{
...
antd::fixed_buffer antd::read_from_stdin_shared_mem()
{
...
void antd::write_redirected_stdout_to_shared_mem()
{
  boost::unique_lock<boost::mutex> scoped_lock(integration_test_mutex);
...
#endif // ANTD_INTEGRATION_TEST_HOOKS_IMPLEMENTATION
